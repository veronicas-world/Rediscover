// Shared tool registration for the Whel Corpus MCP server. Used by both the
// stdio entrypoint (server.mjs) and the HTTP entrypoint (server-http.mjs).
import { z } from "zod";
...
export function registerTools(server) {
  server.registerTool(
    "whel_corpus_meta",
    {
      ...
      description:
        "Overview of the whole Whel corpus: build date, total candidate count, the six conditions covered, and the tier + curation distribution overall and per condition. Call this first to see what is available.",
      inputSchema: {},
    },
  ...
  server.registerTool(
    "whel_list_candidates",
    {
      ...
      description:
        "List drug-condition candidates Whel has surfaced, as compact summaries (drug, condition, curationClass, tier, score, arm, on/off-label, generic availability, trial stage, MATRIX percentile). Defaults to the clean single-agent drug index. Filter by condition, tier, evidence arm, regulatory status, drug substring, or curationClass. Paginated.",
      inputSchema: {
        ...
        curationClass: z.enum(["drug", "combination", "supplement", "exclude", "class", "all"]).optional().describe("defaults to 'drug' (clean single-agent index, what the site shows); 'all' includes combinations/supplements/excluded/class"),
