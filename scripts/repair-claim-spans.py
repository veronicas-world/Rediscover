...
- Return one or two COMPLETE sentences.
- The span must be self-sufficient. Read it with nothing else in front of you and
  it should still establish the claim. That means it has to contain:
    * the INTERVENTION the claim is about, named (an abbreviation the source
      itself uses is fine),
    * any COMPARATOR the claim mentions ("compared with X", "superior to X"),
    * the DIRECTION of the effect (improved / reduced / no difference / harmful /
      inferior), not merely the outcome name or a bare statistic.
  A span that names an outcome and a confidence interval but leaves the drug or
  the direction to be inferred from elsewhere in the paper is not acceptable.
- Subject to that, prefer the shortest span.
...
def write_migration(repairs: list[Repair], path: Path) -> None:
    ...
    unnamed = [r for r in repairs if r.status == "still_unnamed"]
    if unnamed:
        lines.append("-- STILL UNNAMED — after widening, the span does not name the drug by any")
        lines.append("-- spelling this script recognises. Left unchanged. Most are the matcher's")
        lines.append("-- limits rather than real defects (brand names like Cymbalta for duloxetine,")
        lines.append("-- contracted forms like myo-Ins, hyphenated acronyms like LNG-IUS). The rest")
        lines.append("-- are quotes whose grammatical subject sits earlier in the paper. Review:")
        for r in unnamed:
            lines.append(f"--   {r.claim_id}  [{r.source} {r.external_id}]  {comment_safe(r.new_quote, 90)}")
        lines.append("")
...
def main() -> int:
    ...
    parser.add_argument("--missing-drug", action="store_true",
                        help="Target every claim behind an active signal whose quote never names "
                             "the intervention the claim is about, regardless of entailment label. "
                             "These read as entailed only because the judge supplies the drug name "
                             "from somewhere outside the quote.")
    parser.add_argument("--workers", type=int, default=6, help="Parallel API calls. Default 6.")
    parser.add_argument("--tag", default="046",
                        help="Migration number / run tag. Use a fresh one so an earlier run's "
                             "migration and log are not overwritten.")
    args = parser.parse_args()

    run_log_path = REPO / "scripts" / "audit-output" / f"claim-span-repairs-{args.tag}.json"
    migration_path = REPO / "supabase" / "migrations" / f"{args.tag}_repair_claim_spans.sql"
    ...
    if args.missing_drug:
        # A quote that never names its own drug cannot establish a claim about
        # that drug. Whatever label it carries was reached by importing the name
        # from outside the span, which is the thing this whole audit exists to
        # stop. Match on any word of 4+ characters from the intervention label,
        # so "Vitex agnus-castus" is satisfied by a quote saying just "Vitex".
        print("loading interventions…")
        ents = {e["id"]: e["label"]
                for e in sb_get(base, headers, "entities", "select=id,label&type=eq.intervention")}
        claims = sb_get(base, headers, "claims",
                        f"{cols},intervention_id&model_name=eq.claude-sonnet-4-6"
                        "&provenance_verified=is.true")
        claims = [c for c in claims if str(c["id"]) in referenced]
        keep, no_dir = [], 0
        for c in claims:
            label = ents.get(c.get("intervention_id"), "")
            quote = c.get("exact_quote") or ""
            headless = bool(label) and not names_drug(label, quote)
            directionless = not states_direction(quote)
            if headless or directionless:
                if headless:
                    drug_label[str(c["id"])] = label
                if directionless and not headless:
                    no_dir += 1
                keep.append(c)
        claims = keep
        print(f"  {len(claims)} claim(s) with a weak span "
              f"({len(claims) - no_dir} never name the drug, {no_dir} never state a direction)")
    elif args.claim_ids:
    ...
    def process(c: dict) -> Repair:
        ...
        start, end = pos
        # The span was located verbatim, but did widening actually solve the
        # problem? If the drug still isn't named, the source itself carries the
        # subject somewhere this span doesn't reach. Flag rather than accept:
        # a silently-still-headless quote is the exact failure being repaired.
        status = "repaired"
        if drug_label.get(cid) and not names_drug(drug_label[cid], raw[start:end]):
            status = "still_unnamed"
        return Repair(
            claim_id=cid, document_id=doc["id"], source=doc.get("source", "?"),
            external_id=doc.get("external_id", ""), status=status,
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        for r in pool.map(process, targets):
            repairs.append(r)
            if len(repairs) % 20 == 1 or len(repairs) == len(targets):
                print(f"  · {len(repairs)} / {len(targets)} …", flush=True)
            if len(repairs) % 20 == 0:
                ...
                run_log_path.write_text(json.dumps({
                    ...
                    "model": args.model,
                    "status": "in_progress",
                    "results": [asdict(x) for x in repairs],
                }, indent=2) + "\n")
    ...
    write_migration(repairs, migration_path)
