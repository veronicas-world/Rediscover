...
/**
 * OpenFDA AEMS Pipeline (FDA Adverse Event Monitoring System, formerly FAERS)
...
async function fetchAllReports(drugs, debug = false) {
  ...
  for (const drug of drugs) {
    ...
    log(`   → ${generalTotal.toLocaleString()} total female-patient reports in FDA AEMS`);
    log(`   → Pass 1 (gynae-targeted): ${gynaeReports.length} report(s) [${gynaeTotal.toLocaleString()} match in FDA AEMS]`);
...
// ── Format AEMS reports for Claude ───────────────────────────────────────────
...
/**
 * Summarise both pass-1 (condition-targeted) and pass-2 (general) AEMS reports
...
function formatForClaude(drugClass, byDrug) {
  ...
  for (const [drug, { gynaeReports, generalReports, gynaeTotal, generalTotal }] of byDrug) {
    ...
    sections.push(
      ...
      `\n` +
      `-- Pass 1: Condition-targeted reports (${gynaeReports.length} sampled of ${gynaeTotal.toLocaleString()} matching in FDA AEMS) --\n` +
...
const FAERS_SYSTEM_PROMPT = `You are a pharmacovigilance analyst specializing in women's health and drug repurposing. You are reviewing FDA Adverse Event Monitoring System (AEMS, formerly FAERS) data for a drug class.
...
async function analyzeWithClaude(apiKey, drugClass, content) {
  const userMessage =
    ...
    `The following is a two-pass summary of FDA Adverse Event Monitoring System (AEMS, formerly FAERS) data ` +
...
async function generateSQL(drugClass, signals, byDrug, supabaseUrl, supabaseKey) {
  ...
  out.push('-- ================================================================');
  out.push(`-- Rediscover Women — OpenFDA AEMS Pipeline Output`);
  out.push(`-- Drug class : ${drugClass}`);
  out.push(`-- Generated  : ${today}`);
  out.push(`-- Model      : ${MODEL}`);
  out.push(`-- Source     : FDA Adverse Event Monitoring System (AEMS, formerly FAERS)`);
  out.push('-- ================================================================');
  out.push('');

  if (signals.length === 0) {
    out.push('-- No noteworthy signals were identified from the FDA AEMS data.');
    return out.join('\n');
  }
  ...
  for (const s of enriched) {
    ...
    // Delete any existing AEMS sources for this compound+condition so re-runs
...
async function main() {
  ...
  // ── Step 1: Fetch AEMS reports
  log(`\nStep 1 — Querying OpenFDA AEMS for female patients...`);
  ...
  if (totalReports === 0) {
    log('\nNo AEMS reports found. Run with --debug to inspect the raw API response.');
    process.exit(0);
  }
